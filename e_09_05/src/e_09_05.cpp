...
{
	char*	MIN = const_cast<char*>(cmin[0]);	//最小値を代入するための型。
		...
		//もし今の最小値より比較した要素のほうが小さければそれを最小値として採用。
			...
			MIN = const_cast<char*>(cmin[i]);	//最小値を新たに代入。
...
int main()
{
	...
	const char*	cmin[n] = {"ABCD", "EFGH", "HIJKL", "OPQR", "STUV"};	//最小値を比較文字列を宣言。
	...
	//double型の場合
	cout	<<"double型配列dminの中の最小値は"	<<min_of(dmin, n)				<<"です。\n";

	//文字列の場合。
	cout	<<"文字列のうち最小値は"			<<min_of(cmin, n)	<<"です。\n";
