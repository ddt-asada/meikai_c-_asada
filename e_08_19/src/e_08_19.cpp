...
double str2double(const char* s)
{
	char	key[] = "0123456789";	//走査用の文字列。

	char	dot = '.';				//文字列の中から小数点を探し出すために使用する。

	int		dotPlace = 100;			//小数点が格納されていた要素を記憶するのに使用。最後の桁数を下げる回数。小数点が含まれていたかどうかの判定もしたいので-1で初期化。
	...
	int		judge = 0;				//数字が見つかったかどうかを判定するための型。文字列に数字が含まれていれば1を含まれていなければ0を返す。
	...
	for(int i = 0; i < len; i++) {
			...
			if(s[i] == key[j]) {
				...
				if(i > dotPlace) {
					...
					for(int k = len - 1; k > i; k--) {
						...
						dig *= 10;		//10のi乗を求める。先ほどの考えに則り一番後ろは1桁なのでそのまま（このループに突入しない）、2桁目以降には10をかけていく。

						}
				}

				num += j * dig;		//見つけた数字と桁数に応じて10のi乗したものを合計していくことにより、整数に変換していく。

				dig = 1;			//計算に使った値をリセットする。

				judge = 1;			//数字が見つかったことを表す。
		...
		if (s[i] == dot) {
			...
			judge = 1;		//小数点が見つかっても実数値と判断するので判定に1を代入する。
